...
success:function(data)
{
   $("#fetch-feild").html(data);
    ...
    ?>
          ...
          <h1 style="font-family: Montserrat;" class="page-header"><?php echo $row['LASTNAME'] . ', ' . $row['FIRSTNAME']. ' ' . $row['MIDDLENAME'] ?></h1>
          <?php
    } mysqli_close($conn);
      ?>
      <div class="col-md-5">
      ...
      <label style="font-family: Montserrat;" for="focusedInput">Select Grade:</label>
      <select class="form-control" style="height:30px;font-size:12px; font-family: Montserrat;" id="fetch">
        <option > </option>
    ...
    <?php $query = mysqli_query($conn,"SELECT principal,school_year FROM school_year where status='Yes'");
    while($sy = mysqli_fetch_assoc($query)){ ?>
    <a style="font-family: 'Montserrat'; text-align:center; color:white; font-size:13px; border-color:#B10B0B; background-color:#B10B0B;" type="button" class="btn btn-success" href="rms.php?page=Students" > Back</a>
    <a style="font-family: Montserrat; border-color:#273893; background-color:#273893;" href="rms.php?page=record&id=<?php  echo $_GET['id']?>" class="btn btn-success"> Show all record</a>
      <a style="font-family: Montserrat; font-size:15px;" class='btn btn-success' href="rms.php?page=addrecord&id=<?php echo $_GET['id'] ?>&principal=<?php echo $principal['principal'] ?>&sy=<?php echo $sy['school_year'] ?>&prog=<?php echo $_GET['prog'] ?>"> Add grade</a>
      <?php
    } ?>
    </div>
    <br>
